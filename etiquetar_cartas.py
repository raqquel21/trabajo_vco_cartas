...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        if le > 0:
            ...
            print(f'Total: {le} Cartas. Empezando por la carta {self.card_idx}')
        else:
    ...
    def show_suit(self):
        ...
        for ll in car.motifs:
            if ll.label in SUITS:
                car.realSuit = ll.label
                break
        for ll in car.motifs:
            if ll.label in FIGURES:
                car.realFigure = ll.label
                break
    ...
    def show_motif(self, motif):
        ...
        self.label = ttk.Label(self, text=f"Motivo actual: {motif.label} ")
        self.label.grid(column=0, row=1, padx=5, pady=5)
    
    def motif_changed(self, value):
        if (self.motif_idx != -1):
            self.motifs[self.motif_idx].label = value
