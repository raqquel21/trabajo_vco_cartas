       ...
       # contours_roi, hi = cv2.findContours(thresh_roi, cv2.RETR_LIST ,  cv2.CHAIN_APPROX_SIMPLE)
        ...
        for k in range(1, totalLabels_roi):
            ...
            if (area > MIN_AREA) and (area < MAX_AREA):
                ...
                x0 = val[k, cv2.CC_STAT_LEFT]
                y0 = val[k, cv2.CC_STAT_TOP]
                w0 = val[k, cv2.CC_STAT_WIDTH]
                h0 = val[k, cv2.CC_STAT_HEIGHT]
                mu = cv2.moments(cnt)
                if mu['m00'] == 0:
                    continue
                area_motif = cv2.contourArea(cnt)
                perimeter_motif = cv2.arcLength(cnt, True)
                # Momentos de Hu
                ...
                motif.contour = cnt
                motif.motifId = imot
                motif.area = area_motif
                motif.perimeter = perimeter_motif 
                ...
                motif.features.append(float(area_motif))
                motif.features.append(float(perimeter_motif))

                # Bounding box (tamaño)
                motif.features.append(float(w0))
                motif.features.append(float(h0))
                motif.features.append(float(w0 / h0))  # relación de aspecto

                # Centroide (posición relativa)
                motif.features.append(float(centroide[0]))
                motif.features.append(float(centroide[1]))

                # Círculo mínimo
                motif.features.append(float(radious))

                # Momentos invariantes de Hu
                for h in hu:
                    motif.features.append(float(h))
                car.motifs.append(motif)
                # Se añade a la lista de motivos
                imot+=1
...
filecard = 'trainCards.npz'   # 'trainCards.npz  testCards.npz
